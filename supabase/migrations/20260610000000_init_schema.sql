-- =====================================================================
-- AIbou — 初期スキーマ（既存 supabase_schema.sql をマイグレーション化したもの）
-- 新しいSupabaseプロジェクト(ref: hwjmojipsablfevtjzln)へは:
--   supabase link --project-ref hwjmojipsablfevtjzln
--   supabase db push          # ← この初期マイグレーションを適用してテーブルを作成
-- 既にスキーマがあるプロジェクトから取り込む場合は:
--   supabase db pull          # ← リモートの定義をマイグレーションとして取得
-- すべて CREATE ... IF NOT EXISTS なので冪等（既存テーブルがあっても安全）。
-- =====================================================================
