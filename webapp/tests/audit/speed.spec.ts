 ...
 *
 * 1周の計測で「画面を移るのに 1.3〜1.9秒」と出たことがある。中身は
 * 右上のメニュー（Modes）の絵の動きと、画面の部品の読み込みと、
 * サーバーへの問い合わせが混ざっていた。メニューは外した（入口は
 * 実行／管理の1つ）ので、いまは部品の読み込みと問い合わせを分けて出す。
 *
...
/** その画面に切り替わって、中身が出るまで（入口は実行／管理と「もっと」）。 */
async function switchTo(page: Page, mode: string, be: Backend) {
  ...
  return { paint: painted - clicked, calls: be.calls.length - before };
}
